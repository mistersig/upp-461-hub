...
(function(){
  ...
  /* Linear interpolation demo (data-interp)
     Helps students see how address ranges map to a point along a segment.
     Notes:
     - This is a teaching demo (not a full geocoding engine).
     - Many datasets treat even numbers on the left and odd on the right, but not always.
  */
  function initInterpolationDemo(){
    var form = qs('[data-interp]');
    if (!form) return;

    var out = qs('[data-interp-out]', form);
    ...
    function makeNotice(kind, html){
      if (!out) return;
      out.innerHTML = '<div class="notice ' + kind + '">' + html + '</div>';
    }

    function solve(){
      var addrEl = qs('input[name="addr"]', form);
      var lfEl = qs('input[name="lf"]', form);
      var ltEl = qs('input[name="lt"]', form);
      var rfEl = qs('input[name="rf"]', form);
      var rtEl = qs('input[name="rt"]', form);

      var addr = toInt(addrEl ? addrEl.value : '');
      var lf = toInt(lfEl ? lfEl.value : '');
      var lt = toInt(ltEl ? ltEl.value : '');
      var rf = toInt(rfEl ? rfEl.value : '');
      var rt = toInt(rtEl ? rtEl.value : '');

      if ([addr, lf, lt, rf, rt].some(function(x){ return x === null; })){
        makeNotice('warn', '<strong>Enter all numbers</strong> (address + left/right ranges).');
        setLive('Interpolation demo needs all inputs.');
        return;
      }

      var side = (addr % 2 === 0) ? 'left (even)' : 'right (odd)';
      var from = (addr % 2 === 0) ? lf : rf;
      var to = (addr % 2 === 0) ? lt : rt;

      if (from === to){
        makeNotice('bad', '<strong>Range is invalid.</strong> Start and end cannot be the same.');
        setLive('Interpolation demo error: invalid range.');
        return;
      }
      ...
      var frac = (addr - from) / (to - from);
      var pct = Math.round(Math.max(0, Math.min(1, frac)) * 100);

      var within = (addr >= a && addr <= b);
      var warn = within ? '' : '<div class="small"><strong>Note:</strong> ' +
        'Your address is outside the chosen range ('+a+'–'+b+'). A real geocoder would likely return <em>unmatched</em> or look for another candidate street.</div>';

      makeNotice(within ? 'good' : 'warn',
        '<strong>Estimated position:</strong> about <strong>' + pct + '%</strong> of the way from the start of the segment on the <strong>' + side + '</strong> side.' +
        '<div class="small">This is <em>linear interpolation</em>: an estimate of where the address falls within the address range.</div>' +
        warn
      );

      setLive('Interpolation demo computed.');
    }
    ...
    var btn = qs('[data-interp-run]', form);
    if (btn) btn.addEventListener('click', function(){ solve(); });
  }
  ...
  function initBugReport(){
    ...
    var subject = 'UPP 461 bug report (Week 11): ' + title;
